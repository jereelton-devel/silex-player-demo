...
use \SilexPlayer\SilexPlayerQuery;
use \SilexPlayer\SilexPlayerSendPostAPI;
use \SilexPlayer\SilexPlayerStart;
use \SilexPlayer\SilexPlayerStop;
use \SilexPlayer\SilexPlayerStatus;
use \SilexPlayer\SilexPlayerCheckWinner;
...
$app->get('/controll/player-start', function(){
    ...
    $player_start = SilexPlayerStart::startPlayerNow();
...
$app->get('/info/player-status', function(){
    ...
    if(SilexPlayerStatus::checkPlayerStatus() == true) {
        echo 1;
    } else {
        echo 0;
    }

    //file_put_contents('log', date("F j, Y, g:i a")." checkPlayerStatus: ".SilexPlayerStatus::checkPlayerStatus()."\r\n", FILE_APPEND);
...
$app->get('/player_running/{from_api}/{value}/{player_id}', function($from_api, $value, $player_id){
    ...
    if(SilexPlayerStatus::checkPlayerStatus() == true) {
        ...
        if ($from_api == 'api2_val') {
            ...
            $checkWinner = SilexPlayerCheckWinner::getPlayerWinner($winner[0]['api3_val'], $winner[0]['api1_val'], $winner[0]['api2_val']);
            ...
            //Gravar o numero gerado na base de dados
            //$stmt = new SilexPlayerQuery(); //Instanciado no inicio da rota
            $stmt->queryInsertRandomNumber($api3_value,'api3_val', 1);
            if(!$stmt->queryExecute()) {
